...
│
├── benchmarks/                     # 벤치마크 시스템 (배포 제외)
│   ├── __init__.py
│   ├── baseline_libraries.py      # 기존 라이브러리 구현
│   ├── benchmark_record.py        # 벤치마크 기록
│   ├── benchmark_runner.py        # 벤치마크 실행기
│   ├── run_benchmark.py           # 메인 실행 스크립트
│   └── test_cases*.txt            # 테스트 케이스
│
├── report/                         # 벤치마크 결과 (배포 제외)
│   ├── *_benchmark_report.md      # 벤치마크 리포트
│   ├── *_error_report_*.md        # 에러 리포트
│   └── ignore/                    # 이전 리포트
│
├── .github/                        # GitHub 설정
│   ├── workflows/
...
│   └── copilot-instructions.md    # Copilot 가이드
│
